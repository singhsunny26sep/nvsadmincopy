...
import { Plus, Edit, Trash2, MapPin, Navigation, User } from "lucide-react";
import { locationsAPI, usersAPI } from "../../components/api/api";
...
const LocationForm = ({
  ...
  const [formData, setFormData] = useState({
    ...
    userId: location?.userId || (currentUser?._id || currentUser?.id || ""),
  });
          ...
          <div>
            ...
            <div className="relative">
              ...
              <select
                ...
                onChange={handleInputChange}
                className="w-full pl-9 pr-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent appearance-none"
              >
...
const LocationManagement = () => {
  ...
  const fetchUsers = async () => {
    try {
      ...
      const usersParams = { role: "vendor", limit: 100 };

      const usersResponse = await usersAPI.getUsers(usersParams);
      console.log("Users API raw response:", usersResponse.data);

      let usersData = [];
      if (usersResponse.data.success) {
        if (usersResponse.data.data && usersResponse.data.data.data && Array.isArray(usersResponse.data.data.data)) {
          usersData = usersResponse.data.data.data;
        } else if (Array.isArray(usersResponse.data.data)) {
          usersData = usersResponse.data.data;
        } else if (usersResponse.data.data && usersResponse.data.data._id) {
          usersData = [usersResponse.data.data];
